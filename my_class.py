...
class MyClass():
    ...
    def load_csv(self):
        ...
        self.df_poly['region_shape_attributes_json'] = self.df_poly['region_shape_attributes'].apply(json.loads)
        ...
        self.df_poly = self.df_poly.drop(["file_size","file_attributes","region_shape_attributes", "region_attributes", "region_shape_attributes_json", "name"], axis=1)

        # create a filenumber attribute for better file management
        self.df_poly['filenumber'] = self.df_poly["filename"].rank(method='dense').astype(int)

        # create a df specifically for filenames, filenumbers and region counts
        self.df_filenames = self.df_poly.drop_duplicates(['filename', 'filenumber', 'region_count'])[['filename', 'filenumber', 'region_count']].set_index('filenumber')
    ...
    def get_ground_truth_mask(self, filenumber, filename, shape):
        ...
        x_points = self.df_poly.loc[self.df_poly['filenumber'] == filenumber, 'all_points_x'].tolist()
        y_points = self.df_poly.loc[self.df_poly['filenumber'] == filenumber, 'all_points_y'].tolist()
    ...
    def compute_refined_mask(self, img):
        """
        ...
        # Skin detection
        skin_mask = self.skin_detect(img)
        ...
        if((not np.where(human_mask != 255)[0].any()) and (not np.where(human_mask != 255)[0].any())):
            # Uncomment the line below to debug:
            #self.show_images([human_mask, combined_mask], ['Human Mask', 'Combined Mask'], cmap='gray')
            raise NoHumanMaskError("Human Mask was empty, even though a body part was detected.")

        # Refine mask using GrabCut
        refined_mask = self.refine_mask_with_grabcut(img, human_mask)

        return skin_mask, edges, filtered_edges, combined_mask, contours, human_mask, refined_mask

    def remove_human_parts_with_visualization(self, filenumber):
        """
        ...
        try:
            skin_mask, edges, filtered_edges, combined_mask, contours, human_mask, refined_mask = self.compute_refined_mask(img)
        ...
        ground_truth_mask = self.get_ground_truth_mask(filenumber, filename, refined_mask.shape)
    ...
    def mean_iou(self):
        ...
        for filenumber in filenumbers:
            ...
            img = self.load_img(filename)
            try:
                _, _, _, _, _, _, refined_mask = self.compute_refined_mask(img)
